...
int bruteForce(vector<vector<int> > &graph, vector<int> &path, int numNodes); //brute force solution
...
int main() {
    ...
    if (calcMethod == brute) {
        ...
        cost = bruteForce(graph, path, numNodes); 
        ...
        //output path and total distance
        print(path1, numNodes, cost, time, m, outFile);
...
int bruteForce(vector<vector<int> > &graph, vector<int> &path, int numNodes) { //path is passed in as a reference to allow it to be edited in function
    //set up vector to list nodes not including the starting/ending nodde
    vector<int> possiblePath(numNodes - 1); //holds indexes of nodes excluding starting node

    //create path in numerical order
    iota(possiblePath.begin(), possiblePath.end(), 1);
    ...
    do { //do these action while there are possible permutations
        ...
        if (firstTime == true) { //minCost and minPath have not been set
            ...
            //copy path over
            path.assign(possiblePath.begin(), possiblePath.end());
            path.insert(path.begin(), 0); //add starting vector to path 
...
int findPathViaSwapping(vector<vector<int> > &graph, vector<int> &path, int numNodes) {
    ...
    vector<int> minPath(numNodes); //holds shortest path
    vector<int> resetPath(numNodes); //holds path being used to reset path for function
    ...
    for (int loop = 0; loop < numChanges; loop++) { //find swaps and apply them and find shortest one
        ...
        path.clear();
        path.assign(resetPath.begin(), resetPath.end());
    ...
    path.clear();
    path.assign(minPath.begin(), minPath.end());
